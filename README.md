...
确认数据路径和训练参数无误后，从仓库根目录运行：

```bash
python main.py
```
训练完成后，程序会分别在训练集和测试集上计算 RMSE、MAE 和 ACC，并保存以下结果：

- `outputs/hmstr_train.npz`：训练集预测值与真实值；
- `outputs/hmstr_test.npz`：测试集预测值与真实值；
- `output/model/HMSTR/<时间戳>/`：模型权重、模型结构、归一化器、训练历史和评估指标。

如需调整输入长度、预测长度、数据集划分、训练轮数或批大小，请修改 `main.py` 中对应参数。
